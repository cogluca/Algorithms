...
int _edit_distance_strings_dyn(char* string_one, char* string_two) {
    ...
    if(string_one[0] == string_two[0])
        dno_op = _edit_distance_strings_dyn(string_one + 1, string_two + 1);
    else
        dno_op = INT8_MAX;

    dcanc = 1 + _edit_distance_strings_dyn(string_one, string_two + 1);

    dins = 1 + _edit_distance_strings_dyn(string_one + 1, string_two);
...
int edit_distance_strings_dyn(char* one_string, char* another_string) {
    ...
    if(one_string == NULL || another_string == NULL) {
     ...
     return _edit_distance_strings_dyn(one_string, another_string);

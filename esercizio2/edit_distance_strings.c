    ...
    int internal_edit_distance_strings(char* string_one, char* string_two) {
        ...
        if(string_one[0] == string_two[0])
            dno_op = internal_edit_distance_strings(string_one + 1, string_two + 1);
        else
            dno_op = INT8_MAX;

        dcanc = 1 + internal_edit_distance_strings(string_one, string_two + 1);

        dins = 1 + internal_edit_distance_strings(string_one + 1, string_two);
    ...
    int edit_distance_strings(char* one_string, char* another_string) {
        ...
        if(one_string == NULL || another_string == NULL) {
         ...
         return internal_edit_distance_strings(one_string, another_string);
